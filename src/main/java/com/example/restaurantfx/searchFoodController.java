...
public class searchFoodController {
    ...
    public ObservableList<foodExtended> loadFoodItemsByPrice() {
        ...
        try {
            List<List<food>> foodList = restaurantManagerObj.searchByFoodPriceRange(minPrice, maxPrice);
            for (var foodItem : foodList) {
                if (foodItem != null) {
                    for (var food : foodItem) {
                        if (food != null) {
                            foodItems.add(new foodExtended(food, main));
                        }
                    }
                }
            }
    ...
    public ObservableList<foodExtended> loadFoodItemsByCategory() {
        ...
        try {
            List<List<food>> foodList = restaurantManagerObj.searchByFoodCategory(cat);
            for (var foodItem : foodList) {
                if (foodItem != null) {
                    for (var food : foodItem) {
                        if (food != null) {
    ...
    public ObservableList<foodExtended> loadFoodItemsByName() {
        ...
        List<List<food>> foodList = restaurantManagerObj.searchByName(name);
        for(var foodItem : foodList){
            if(foodItem!=null){
                for( var food: foodItem){
                    if(food!=null) {
